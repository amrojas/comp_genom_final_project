...
python3 $APP_ROOT/src/main.py --datafiles synthetic-large.fastq --auto --create-cuckoo-filter -e $EXPECTED_ITEMS -p $FP_PROB -q fp_dataset.fastq --fp-query &>> $OUTPUT1
echo "experimenting with e=$EXPECTED_ITEMS, p=$p, cuckoo-bit"
python3 $APP_ROOT/src/main.py --datafiles synthetic-large.fastq --auto --create-cuckoo-filter-bit -e $EXPECTED_ITEMS -p $FP_PROB -q fp_dataset.fastq --fp-query &>> $OUTPUT1
echo "experimenting with e=$EXPECTED_ITEMS, p=$p, cuckoo-stash"
python3 $APP_ROOT/src/main.py --datafiles synthetic-large.fastq --auto --create-cuckoo-filter --stash 50 -e $EXPECTED_ITEMS -p $FP_PROB -q fp_dataset.fastq --fp-query &>> $OUTPUT1
